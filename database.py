...
import os  # ✅ Added for dynamic DB path (optional)

DB_NAME = os.getenv("DATABASE_PATH", "nullprotocol.db")  # ✅ Render persistent disk ke liye
...
async def init_db():
    async with aiosqlite.connect(DB_NAME, timeout=30) as db:  # ✅ timeout added
        # Users table
        await db.execute("""
...
async def redeem_discount_code(user_id, code, plan_id):
    async with aiosqlite.connect(DB_NAME, timeout=30) as db:
        # Check if already used by this user for this plan? (optional)
